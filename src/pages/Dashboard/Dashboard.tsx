import { FC } from "react";
import "./Dashboard.scss";
import { Card } from "../../components/Card/Card";
import { cardData1, cardData2, recentOrders } from "../../data/data";
import graphBar from "../../assets/graphBar.png";
import { useSelector } from "react-redux";

export const Dashboard: FC = () => {
  const isSidebarMinimized = useSelector(
    (state: { general: { isSidebarMinimized: boolean } }) =>
      state.general.isSidebarMinimized
  );

  return (
    <div
      className={
        isSidebarMinimized ? "dashboard-minimized dashboard" : "dashboard"
      }
    >
      <div className="container">
        <div className="dashboard__card">
          {cardData1.map((card) => {
            return (
              <Card
                key={card.icon}
                icon={card.icon}
                heading1={card.heading1}
                heading2={card.heading2}
                heading3={card.heading3}
                content1={card.content1}
                content2={card.content2}
                content3={card.content3}
                bonus1={card.bonus1}
                bonus2={card.bonus2}
              />
            );
          })}
        </div>

        <div className="dashboard__wrapper">
          <div className="dashboard__wrapper--left">
            <div className="dashboard__wrapper--left-marketting">
              <div className="dashboard__wrapper--left-marketting-stats">
                <div className="card__top">
                  <p className="dashboard__heading">Marketting</p>

                  <div>
                    <p>This Week</p>

                    <svg
                      width="12"
                      height="8"
                      viewBox="0 0 12 8"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M1 1.5L6 6.5L11 1.5"
                        strokeWidth="2"
                        strokeLinecap="round"
                        strokeLinejoin="round"
                      />
                    </svg>
                  </div>
                </div>

                <div className="dashboard__wrapper--left-marketting-stats-bullets">
                  <div className="dashboard_wrapper--left-_marketting-stats-bullets-1">
                    <div />
                    <p>Acquisition</p>
                  </div>

                  <div className="dashboard__wrapper--left-marketting-stats-bullets-2">
                    <div />
                    <p>Purchase</p>
                  </div>

                  <div className="dashboard__wrapper--left-marketting-stats-bullets-3">
                    <div />
                    <p>Retention</p>
                  </div>
                </div>

                <svg
                  width="205"
                  height="205"
                  viewBox="0 0 205 205"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                  className="dashboard__wrapper--left-marketting-stats-chart"
                >
                  <path
                    d="M205 102.5C205 159.109 159.109 205 102.5 205C45.8908 205 0 159.109 0 102.5C0 45.8908 45.8908 0 102.5 0C159.109 0 205 45.8908 205 102.5ZM56.7908 102.5C56.7908 127.744 77.2555 148.209 102.5 148.209C127.744 148.209 148.209 127.744 148.209 102.5C148.209 77.2555 127.744 56.7908 102.5 56.7908C77.2555 56.7908 56.7908 77.2555 56.7908 102.5Z"
                    fill="#EEF0FA"
                  />
                  <path
                    d="M103 17C82.4661 17 62.6098 24.3471 47.0215 37.7129C31.4331 51.0787 21.1415 69.581 18.0075 89.8744C14.8736 110.168 19.1041 130.913 29.9341 148.358C40.7642 165.804 57.479 178.799 77.0563 184.993L83.2715 165.351C68.3842 160.64 55.6738 150.758 47.4382 137.492C39.2027 124.226 35.9856 108.451 38.3688 93.0188C40.752 77.587 48.5781 63.5173 60.432 53.3535C72.2859 43.1896 87.3853 37.6026 103 37.6026V17Z"
                    fill="#FFCC91"
                  />
                  <path
                    d="M38.0163 46.6696C46.1641 37.2701 56.2541 29.75 67.5901 24.6282C78.9261 19.5064 91.2382 16.9047 103.677 17.0027L103.515 37.6047C94.0559 37.5302 84.6934 39.5086 76.0731 43.4034C67.4528 47.2982 59.7801 53.0167 53.5842 60.1644L38.0163 46.6696Z"
                    fill="#97A5EB"
                  />
                  <path
                    d="M103 17C115.474 17 127.798 19.7135 139.118 24.9522C150.439 30.1909 160.484 37.8295 168.557 47.3382C176.631 56.8468 182.539 67.9979 185.872 80.018C189.206 92.038 189.884 104.639 187.861 116.948C185.838 129.257 181.162 140.978 174.157 151.298C167.151 161.619 157.985 170.293 147.292 176.717C136.6 183.141 124.639 187.163 112.237 188.502C99.8355 189.842 87.291 188.468 75.4735 184.476L82.0679 164.957C91.0543 167.993 100.594 169.038 110.024 168.019C119.455 167 128.551 163.942 136.681 159.057C144.812 154.172 151.783 147.576 157.11 139.728C162.437 131.88 165.993 122.966 167.532 113.606C169.07 104.247 168.554 94.6641 166.019 85.5237C163.484 76.3832 158.991 67.9035 152.852 60.6728C146.713 53.4421 139.074 47.6335 130.466 43.6498C121.857 39.6661 112.485 37.6026 103 37.6026V17Z"
                    fill="#5570F1"
                  />
                </svg>
              </div>

              <div className="dashboard__wrapper--left-marketting-cards">
                {cardData2.map((card) => {
                  return (
                    <Card
                      key={card.icon}
                      icon={card.icon}
                      heading1={card.heading1}
                      heading2={card.heading2}
                      content1={card.content1}
                      content2={card.content2}
                      bonus1={card.bonus1}
                      bonus2={card.bonus2}
                    />
                  );
                })}
              </div>
            </div>

            <div className="dashboard__wrapper--left-summary">
              <div className="dashboard__wrapper--left-summary-top">
                <div className="dashboard__wrapper--left-summary-top-left">
                  <p className="dashboard__heading">Summary</p>

                  <div className="dashboard__wrapper--left-summary-top-left-sales">
                    <p>Sales</p>
                    <svg
                      width="12"
                      height="8"
                      viewBox="0 0 12 8"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M1 1.5L6 6.5L11 1.5"
                        stroke="#5570f1"
                        strokeWidth="2"
                        strokeLinecap="round"
                        strokeLinejoin="round"
                      />
                    </svg>
                  </div>
                </div>

                <div className="dashboard__wrapper--left-summary-top-dropdown">
                  <p>Last 7 days</p>

                  <svg
                    width="12"
                    height="8"
                    viewBox="0 0 12 8"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M1 1.5L6 6.5L11 1.5"
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                    />
                  </svg>
                </div>
              </div>

              <img src={graphBar} alt="Graph Bar" />
            </div>
          </div>

          <div className="dashboard__wrapper--recent">
            <p className="dashboard__heading">Recent Orders</p>

            {recentOrders.map((order, idx) => {
              return (
                <div
                  className="dashboard__wrapper--recent-order"
                  key={order.status + idx}
                >
                  <img src={order.image} alt="Order Image" />

                  <div className="dashboard__wrapper--recent-order-text">
                    <div className="dashboard__wrapper--recent-order-text-top">
                      <p className="dashboard__wrapper--recent-order-text-top-phone">
                        iPhone 13
                      </p>
                      <p className="dashboard__wrapper--recent-order-text-top-date">
                        12 Sept 2022
                      </p>
                    </div>

                    <div className="dashboard__wrapper--recent-order-text-bottom">
                      <p className="dashboard__wrapper--recent-order-text-bottom-price">
                        ₦730,000.00 x 1
                      </p>

                      <div className={order.status}>
                        <p>{order.status}</p>
                      </div>
                    </div>
                  </div>
                </div>
              );
            })}
          </div>
        </div>
      </div>
    </div>
  );
};
